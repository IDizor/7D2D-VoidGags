...
using HarmonyLib;
using UnityEngine;
using static VoidGags.VoidGags.ExperienceRewardByMaxHP;
...
namespace VoidGags
{
    ...
    {
        public void ApplyPatches_ExperienceRewardByMaxHP()
        {
            ...
            if (Settings.ExperienceRewardByMaxHP_Multiplier >= 0)
            {
                ...
                Harmony.Patch(AccessTools.Method(typeof(EntityPlayer), nameof(EntityPlayer.AddKillXP)),
                    prefix: new HarmonyMethod(EntityPlayer_AddKillXP.Prefix),
                    postfix:new HarmonyMethod(EntityPlayer_AddKillXP.Postfix));
        ...
        public static class ExperienceRewardByMaxHP
        {
            public static float ExpMultiplier = 1.0f;
            public static ConcurrentDictionary<string, int> EntityExperience = new();

            /// <summary>
            /// Experience reward for killing zombies depends on their max health and armor rate. Plus a little random.
            /// </summary>
            public static class EntityPlayer_AddKillXP
            {
